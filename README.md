...
.
├── src/
│   └── index.ts          # Plugin implementation and public types
├── __tests__/
│   ├── build.test.ts     # Integration tests that run real Vite builds
│   └── src/              # Sample HTML fixtures used by the tests
├── dist/                 # Compiled output (generated by the build, gitignored)
├── package.json          # Package metadata and npm scripts
├── tsconfig.json         # TypeScript configuration
├── vitest.config.ts      # Vitest projects + V8 coverage configuration
└── pnpm-lock.yaml        # pnpm lockfile
```

The plugin is a single default export (`vitePluginGlobInput`) defined in `src/index.ts`. It runs only during `build` (`apply: 'build'`) with `enforce: 'pre'`, resolving glob patterns via fast-glob inside Rollup's `options` hook and merging the matched files into `build.rollupOptions.input`.
...
# Run tests in watch mode (Vitest default when a TTY is attached)
npx vitest
```
...
# Compile TypeScript to dist/ (declarations + source maps)
npm run tsc
...
> This repository uses [pnpm](https://pnpm.io/) as its package manager (see `pnpm-lock.yaml`). Run `pnpm install` to set up dependencies; the `npm` commands above map to the same scripts defined in `package.json`.
